...
## Root semantics
- This card is the triage/final-aggregation root, not the broad implementation worker.
- Keep it in triage until the bounded child graph, ownership, dependencies, profile routing, workspace, retry/runtime policy, and acceptance gates are persisted and verified.
- The implementation must be assigned to `default`; do not silently route any child to another profile.

## Required bounded decomposition
1. Project scaffold, astronomical data model, NASA/JPL source manifest, scale manager, and README.
2. Three.js scene/core implementation: celestial hierarchy, logarithmic heliocentric and satellite scales, elliptical/inclined orbit rendering, simulation clock, procedural materials, Sun lighting, Saturn/Uranus rings, and star field.
3. Interaction/UI implementation: OrbitControls, camera focus transitions, raycasting selection, HUD/control panel, labels, scale disclaimer, and information panel.
4. Integration and validation: responsive desktop/mobile layout, npm install/dev/build/typecheck checks, runtime console-error check, interaction smoke test, and final acceptance evidence.

Each child must have non-overlapping owned paths, forbidden paths, explicit dependencies, exact validation commands, durable artifact paths, and a full-hash commit handoff. Link the terminal validation/integration child to this root as the final aggregation gate; do not make the root an execution prerequisite for its own children.

## Required acceptance gates
- The project runs with `npm install` and `npm run dev`.
- Sun through Pluto and all required major moons are represented.
- Real astronomical data remains separate from logarithmic distance and visibility-oriented size rendering.
- Elliptical/inclined orbits and period ratios are observable.
- Focus/detail view, labels, information panel, scale selectors, simulation controls, and scale disclaimer work.
- Saturn rings are present; procedural assets do not depend on external image resources.
- Desktop/mobile layout, TypeScript compilation, runtime console, resize handling, and resource disposal are validated.
- README documents data sources, formulas, physical-vs-rendered scale distinction, moon selection, and execution.

## Completion protocol
Workers must report changed files, forbidden-path check, validation commands and exit codes, artifact/evidence paths, assumptions, full commit hash, and remaining risks. The worker must finish with a successful `kanban_complete`; do not claim completion from a natural-language response alone.
